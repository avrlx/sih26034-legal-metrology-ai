...
def is_present(value):
    ...
    if isinstance(value, dict):
        return any(v is not None and str(v).strip() for v in value.values())
    return True
...
def validate_net_quantity(value):
    ...
    allowed_units = {"G", "KG", "ML", "L", "N", "U", "CM", "M"}
...
def validate_mrp_netqty_contrast(value):
    """Evaluate deterministic prototype contrast evidence.

    Cut-offs are implementation-defined engineering thresholds, not statutory
    Legal Metrology thresholds.
    """
    if not isinstance(value, dict):
    ...
    if unreliable:
        return "REVIEW", "Contrast evidence is not reliable enough for: " + ", ".join(unreliable)
    missing_metrics = [
        measurement.get("target", name)
        for name, measurement in zip(required, measurements)
    ...
    if missing_metrics:
        return "REVIEW", "Contrast metrics are incomplete for: " + ", ".join(missing_metrics)
    strong = [
        measurement["contrast_ratio"] >= 3.0 or measurement["lab_color_difference"] >= 35.0
    ...
    if all(strong):
        return "PASS", "Both declarations meet the implementation-defined engineering contrast threshold (not a statutory threshold)"
    if any(low):
        failed = [measurement.get("target", required[index]) for index, measurement in enumerate(measurements) if low[index]]
        return "FAIL", "Low contrast detected for " + ", ".join(failed) + " under implementation-defined engineering thresholds (not statutory thresholds)"
    return "REVIEW", "Contrast is borderline under implementation-defined engineering thresholds; human review is required"
...
def validate_contrast_target(value, target_name=None):
    ...
    if ratio >= 3.0 or color_difference >= 35.0:
        return "PASS", f"{name} meets an implementation-defined engineering contrast threshold (not a statutory threshold)"
    if ratio < 1.5 and color_difference < 12.0 and confidence >= 0.75:
        return "FAIL", f"{name} is below implementation-defined engineering contrast thresholds (not statutory thresholds)"
    return "REVIEW", f"{name} contrast is borderline; human review is required"


def evaluate_small_package_exemption(value):
    if not isinstance(value, dict):
        return "REVIEW", "Net quantity unavailable; exemption cannot be determined"
    quantity = value.get("value")
    unit = str(value.get("unit", "")).upper()
    if quantity is None:
        return "REVIEW", "Quantity unavailable"
    if unit == "G" and quantity <= 10:
        return "PASS", "Package falls within <=10 g exemption condition"
    if unit == "ML" and quantity <= 10:
        return "PASS", "Package falls within <=10 ml exemption condition"
    return "NOT_APPLICABLE", "Package does not meet small-package exemption condition"


def _font_height_threshold_mm(area_cm2, *, formed=False):
    """Return amended Rule 7 Table-I minimum for principal display panel area."""
    if area_cm2 <= 50:
...
def evaluate_font_height_applicability(fields):
    ...
    if not isinstance(quantity, dict):
        return "REVIEW", "Net quantity unavailable"
    unit = str(quantity.get("unit", "")).upper()
    if unit in {"N", "U"}:
        return "NOT_APPLICABLE", "This prototype font-height rule applies to weight/volume quantities"
    if unit not in {"G", "KG", "ML", "L"}:
        return "REVIEW", f"Unknown quantity unit: {unit}"

    measurement = fields.get("font_height_measurement") or fields.get("glyph_measurement")
    if not isinstance(measurement, dict):
        return "REVIEW", "Numeral-height measurement evidence is unavailable"
    if measurement.get("status") != "OK":
        return "REVIEW", measurement.get("reason") or "Numeral-height measurement is not reliable"
    measured = measurement.get("estimated_numeral_height_mm")
    confidence = measurement.get("measurement_confidence", measurement.get("confidence"))
    if not isinstance(measured, (int, float)) or measured <= 0:
        return "REVIEW", "Numeral-height measurement is invalid"
    if not isinstance(confidence, (int, float)) or confidence < 0.65:
        return "REVIEW", "Numeral-height measurement confidence is too low"

    area = fields.get("principal_display_panel_area_cm2")
    if not isinstance(area, (int, float)):
        return "REVIEW", "Principal display panel area is required to apply amended Rule 7 Table-I"
    if area <= 0:
        return "REVIEW", "Principal display panel area must be greater than zero"

    formed = bool(fields.get("package_surface_formed"))
    threshold = _font_height_threshold_mm(float(area), formed=formed)
    if measured >= threshold:
        return "PASS", f"Measured numeral height {measured:.3f} mm meets Rule 7 Table-I minimum {threshold:.1f} mm for a {area:.1f} cm² principal display panel"
    return "FAIL", f"Measured numeral height {measured:.3f} mm is below Rule 7 Table-I minimum {threshold:.1f} mm for a {area:.1f} cm² principal display panel"
...
def validate_unit_scale(value):
    ...
    if unit in {"N", "U"}:
        return "NOT_APPLICABLE", "Weight/volume unit-scale rule does not apply to count quantity"
    if unit == "G":
        if quantity < 1000:
...
def evaluate_rule(rule, fields):
    ...
    if field_name in {"manufacturer_name", "manufacturer_address", "commodity_name", "country_of_origin"}:
        ...
        if is_present(value) and not has_low_confidence(organization):
            return {"status": "PASS", "reason": f"{field_name} detected", "value": value}
        reason = f"{field_name} has low OCR/extraction confidence" if is_present(value) else f"{field_name} could not be reliably determined"
        return {"status": "REVIEW", "reason": reason, "value": value}
    ...
    if field_name == "mrp":
        ...
        status, reason = validate_unit_scale(fields.get("net_quantity"))
        return {"status": status, "reason": reason, "value": fields.get("net_quantity")}
    elif field_name == "applicability_small_package":
        status, reason = evaluate_small_package_exemption(fields.get("net_quantity"))
        return {"status": status, "reason": reason, "value": fields.get("net_quantity")}
        ...
        return {"status": "REVIEW", "reason": f"Automatic validator not implemented for field: {field_name}", "value": value}
...
def evaluate_compliance(fields):
    ...
    return {"overall_status": overall_status, "summary": summary, "results": results}
